...
WaveformIntegral::WaveformIntegral(WFD5Waveform* waveform, double nsigma, int search_method, std::tuple<int,int> presample_config, int seed_index, int seeded_search_window) : DataProduct()
    ...
    ,clipped_integration_window(false)
    ,detectorSystem(waveform->detectorSystem)
    ,subdetector(waveform->subdetector)
    ,is_time_corrected(false)
    ,time_shift(0)
    ,time_scale(0)
{
...
float WaveformIntegral::IntegralAroundPeak(int nPresamples, int nPostsamples, bool subtract_pedestal)
{
    ...
    auto index1 = 0;
    if (nPresamples < peak_time)
        index1 = peak_time - nPresamples;
    auto index2 = waveform->trace.size();
    if (nPostsamples + peak_time < index2 )
        index2 = peak_time + nPostsamples;

    auto this_integral = std::accumulate(waveform->trace.begin()+index1, waveform->trace.begin()+index2, 0);
    if (subtract_pedestal)
        this_integral -= pedestalLevel*(index2-index1);  // remove the pedestal
...
{
    time_shift = ((WFD5Waveform*)(raw.GetObject()))->digitizationShift * ((WFD5Waveform*)(raw.GetObject()))->digitizationFrequency/40;
...
void WaveformIntegral::Show() const {
    ...
    oss << "        trace: ";
    for (const auto & sample : ((dataProducts::WFD5Waveform*)raw.GetObject())->trace) {
        oss << sample << ", ";
    }
    oss << std::endl;
    std::cout << oss.str();
